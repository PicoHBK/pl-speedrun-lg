...
function buildPlayers(juegos: Juego[]): PlayerData[] {
  ...
  for (const juego of juegos) {
    for (const runner of juego.runners) {
      ...
      map.get(runner.nombre)!.push({
        juego,
        esRecord: runner.esRecord,
  ...
  return Array.from(map.entries())
    ...
    .sort((a, b) => {
      const recsA = a.runs.filter((r) => r.esRecord).length;
      const recsB = b.runs.filter((r) => r.esRecord).length;
      return recsB - recsA;
    });
}
...
function RunDot({
  ...
  return (
    <div
      ...
      style={{ zIndex: 0 }}
      onMouseEnter={() => {
        if (!run.esRecord && recordHolder) onHighlight(recordHolder.nombre);
      }}
      onMouseLeave={() => onHighlight(null)}
    >
      <div
        className={`w-3.5 h-3.5 rounded-full transition-transform duration-150 group-hover/dot:scale-125 cursor-default ${
          run.esRecord
            ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.7)]"
            : "bg-blue-900 border border-blue-700/60"
        }`}
      />
      <div
...
function PlayerRow({
  ...
  return (
    <tr
      className={`border-b border-blue-500/10 last:border-0 transition-colors group/row ${
        isHighlighted
          ? "bg-yellow-500/10 border-l-2 border-l-yellow-400/60"
          : "hover:bg-blue-500/5"
      }`}
    ...
    >
      ...
      <td className="py-3 px-3 text-center">
        <span className="text-xs font-mono text-blue-400/50">{totalRuns}</span>
      </td>
    ...
    <DialogHeader className="border-b border-blue-500/20 pb-3">
      <div className="flex items-center justify-between gap-4">
        <DialogTitle className="flex items-center gap-2 text-blue-100">
          ...
          <div className="flex items-center gap-4 text-xs text-blue-400/50 font-mono ml-2">
